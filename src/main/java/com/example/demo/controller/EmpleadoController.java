...
public class EmpleadoController {
	...
	@PostMapping
	public ResponseEntity<Empleado> create(@Valid @RequestBody Empleado c){
		try {
			Empleado emple= empleadoService.create(c); 
	...
	public ResponseEntity<Empleado> getempleado(@PathVariable("id") Long id){
		Optional<Empleado> emplea= empleadoService.read(id); 
		return emplea.map(vale -> new ResponseEntity<>(vale, HttpStatus.OK) )
				.orElseGet(()-> new ResponseEntity<>(HttpStatus.NOT_FOUND)); 
	}
	
	@PutMapping("/{id}")
	public ResponseEntity<Empleado> updateempleado(@PathVariable("id") Long id, @Valid @RequestBody Empleado empleado){
		 Optional<Empleado> existingempleado = empleadoService.read(id);
	        if (existingempleado.isPresent()) {
	            empleado.setId(id);
	            return new ResponseEntity<>(empleadoService.update(empleado), HttpStatus.OK);
	        } else {
